...
object CatalogEntry {
  ...
  private case class Url(id: String, interface: Interface, regionId: String, url: Uri)
...
case class CatalogEntry(`type`: String, serviceId: String, serviceName: String, endpoints: List[WithId[Endpoint]]) {
  lazy val urisPerRegionPerInterface: Map[String, Map[Interface, Uri]] = endpoints.groupBy(_.regionId).view.mapValues { perRegion =>
    perRegion.groupMap(_.interface)(_.url).view.mapValues(_.head).toMap
  }.toMap
  def urlOf(region: String, interface: Interface): Option[Uri] = urisPerRegionPerInterface.get(region).flatMap(_.get(interface))
  def urlsOf(region: String): Option[Map[Interface, Uri]] = urisPerRegionPerInterface.get(region)
}